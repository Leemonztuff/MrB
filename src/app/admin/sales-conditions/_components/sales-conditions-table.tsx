    ...
    <>
      ...
      <div className="grid gap-3 sm:hidden">
        {salesConditions.map((condition) => (
          <GlassCard key={condition.id} className="hover:bg-white/5 transition-all duration-300">
            <GlassCardHeader className="pb-3 text-center">
              <GlassCardTitle className="leading-none">{condition.name}</GlassCardTitle>
              <GlassCardDescription>{condition.description}</GlassCardDescription>
            </GlassCardHeader>
            ...
            <CardFooter className="flex justify-center gap-3 p-4 pt-0">
              ...
              <ConfirmActionDialog
                title="¿Estás seguro?"
                description="Esta acción no se puede deshacer. Esto eliminará permanentemente la condición."
                confirmLabel="Eliminar"
                pendingLabel="Eliminando..."
                onConfirm={() => handleDelete(condition.id)}
              >
